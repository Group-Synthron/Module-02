...
import CatchDetailsCard from './CatchDetailsCard'; // Import the new component
...
function CatchManager() {
  ...
  const handleQuery = async (event) => {
    // This function can be called without an event, so check for it
    if (event) event.preventDefault();
    setMessage(`Querying for catch ${catchId}...`);
    setQueryResult(null);
    try {
      const response = await fetch(
        `http://localhost:3001/api/catches/${catchId}`
      );
  ...
  const handleTransfer = async (event) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:3001/api/catches/${catchId}/transfer`,
        {
          method: 'PUT',
  ...
  const handleProcessing = async (event) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:3001/api/catches/${catchId}/process`,
        {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
  ...
  const handleWholesale = async (event) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:3001/api/catches/${catchId}/wholesale`,
        {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
      ...
      const data = await response.json();
      if (!response.ok)
        throw new Error(data.error || 'Failed to add wholesale details.');
      setMessage(data.message);
  ...
  return (
    <div className="component">
      ...
      <form onSubmit={handleQuery}>
        <input
          type="text"
          placeholder="Enter Catch ID (e.g., CATCH1)"
          value={catchId}
      ...
      {queryResult && (
        <div>
          ...
          <CatchDetailsCard catchData={queryResult} />

          {/* The forms for actions remain the same */}
          <form onSubmit={handleTransfer}>
            <h4>Transfer Ownership</h4>
            <input
              type="text"
              placeholder="New Owner ID (e.g., ProcessorOrgMSP)"
              value={newOwner}
          ...
          <form onSubmit={handleProcessing}>
            ...
            <input
              type="text"
              placeholder="Processor Name (e.g., Global Fish Inc.)"
              value={processorName}
          ...
          <form onSubmit={handleWholesale}>
            ...
            <input
              type="number"
              placeholder="Wholesale Price per Kg"
            ...
            <input
              type="text"
              placeholder="Batch Split Details"
              value={batchSplitDetails}
              onChange={(e) => setBatchSplitDetails(e.target.value)}
              required
            />
            <button type="submit">Add Details</button>
          </form>
